...
def shapreg_shapley(model, data_loader, reference): # , shapley_value_gt, shapley_ranking_gt):
    ...
    for index, (x, y, z, sh_gt, rk_gt) in enumerate(data_loader):
        ...
        for idx in range(args.circ_num):
            t0 = time.time()
            # attr = permuation_sampling.shapley_value_estimation(z)
...
if __name__ == "__main__":
    ...
    if args.softmax:
        ...
        save_checkpoint_name = "../credit-dataset/softmax/permutation_" + "att_"*args.antithetical + "credit_m_1_s_" + str(args.sample_num) + "_r_" + str(checkpoint["round_index"]) + "_c_" + str(args.circ_num) + ".pth.tar"
        ...
        save_checkpoint_name = "../credit-dataset/wo_softmax/permutation_" + "att_"*args.antithetical + "credit_m_1_s_" + str(args.sample_num) + "_r_" + str(checkpoint["round_index"]) + "_c_" + str(args.circ_num) + ".pth.tar"
